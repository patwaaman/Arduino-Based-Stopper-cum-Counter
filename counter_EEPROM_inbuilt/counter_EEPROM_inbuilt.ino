               ...
               {   
                  key = takeInput(activeButton);
                    if(key == UP)
                        {
                             stopper[i]+= 10;
                             stopper[i] = constrain(stopper[i] , 0 , INT_MAX);
                             lcd.setCursor(0,1);
...
void decrementer()
{       //int x = EEPROM.get(100,x);
        
        //Serial.print(" x = ");
        //Serial.println(x);
        
        int i=EEPROM.read(5) , j=EEPROM.get(100,j) , key = 0;
        ...
        if(key==RIGHT )
         ...
         {   
             ...
             {        
                       ...
                       int stopper = EEPROM.get( temp+(2*(i-1)) , stopper);
                       ...
                       {  
                           lcd.setCursor(0,1);
                           int buttonstate = digitalRead(ok_button);
                              if(buttonstate == LOW )
                              {
                                  j++ ;
                                  lcd.print(j-1);
